...
func main()  {
	// scanner := bufio.NewScanner(strings.NewReader(base))

	// var listOfPhrases [][]string
	// for scanner.Scan() {
	// 	chars := strings.Split(scanner.Text(), "")		
	// 	listOfPhrases = append(listOfPhrases,chars)
	// }
	
	// // fmt.Print(listOfPhrases)
	
	otherList := loadAsChars(base)
	fmt.Println(otherList)

	// sum := 0
	// for index, line := range otherList {
	// 	first := 0
	// 	last := 0
	...
	// 	v1 := strconv.Itoa(first)
	// 	v2 := strconv.Itoa(last)

		
		
	// 	total, _ := strconv.Atoi(v1+v2)
	// 	sum += total

	// 	fmt.Println("\n")
	// 	fmt.Println(listOfPhrases[index])
	// 	fmt.Println(v1, " - ", v2)
	// 	fmt.Println("merged:",total)
	// 	fmt.Println("sum:",sum)
	// 	fmt.Println("\n")
...
// func loadAsChars(text string) [][]string{
// 	scanner := bufio.NewScanner(strings.NewReader(base))

// 	var listOfPhrases [][]string
// 	for scanner.Scan() {
// 		chars := strings.Split(replaceDigits(scanner.Text()), "")		
// 		listOfPhrases = append(listOfPhrases,chars)
// 	}
// 	return listOfPhrases
// }

func loadAsChars(text string) int{
	scanner := bufio.NewScanner(strings.NewReader(base))

	sum := 0
	for scanner.Scan() {
		fmt.Println("----------------------")
		value1, value2 := replaceDigits(scanner.Text())
		total, _ := strconv.Atoi(value1+value2)
		sum += total

		fmt.Println("value1: ", value1)
		fmt.Println("value2: ", value2)
		fmt.Println("calibration value: ", value1+value2)
...
func replaceDigits(phrase string) (string, string) {
	...
	for k, v := range digits {
			new := strings.Replace(phrase, k, strconv.Itoa(v), -1)

			strings.Index(phrase, k)

			phrasesOptions = append(phrasesOptions, new)
			
			firstIndex, lastIndex := getIndexOfFirstAndLastDigit(strings.Split(new, ""))

			
			fmt.Println("firstIndex: ", firstIndex)
			fmt.Println("lastIndex: ", lastIndex)
			// fmt.Println("")
			// fmt.Println("lastIndex: ",lastIndex, string(new[lastIndex]))

			// fmt.Println("firstPos: ", firstPos, string(new[firstIndex]))
			// fmt.Println("lastPos: ", lastPos, string(new[lastIndex]))


			// _, errFirst := strconv.Atoi(string(new[firstIndex]))

			if (firstIndex <= firstPos && firstIndex != -1 ) || (firstPos == -1  && firstIndex != -1) {
				// fmt.Println("first: ", string(new[firstIndex]))

				firstPos = firstIndex
				firstDigit = string(new[firstIndex])
				// fmt.Println("firstPos: ", firstPos)
			}

			// _, errLast := strconv.Atoi(string(new[lastIndex]))
			// if lastIndex >= lastPos || lastPos == -1 {
			if (lastIndex >= lastPos && lastIndex != -1 ) || (lastPos == -1  && lastIndex != -1) {
				
				lastPos = lastIndex
				lastDigit = string(new[lastIndex])

				fmt.Println("last: ", string(new[lastIndex]))
				fmt.Println("lastPos: ", lastPos)
				fmt.Println("lastDigit: ", lastDigit)
			}
	}

	// firstDigit = string(new[firstPos])
	// lastDigit = string(new[lastPos])

	fmt.Println("IdentifiedDigits: ", firstDigit, lastDigit)
	// fmt.Print("Press 'Enter' to continue...")
  // bufio.NewReader(os.Stdin).ReadBytes('\n')

	// fmt.Println(phrasesOptions)
	return firstDigit, lastDigit
}

func getIndexOfFirstAndLastDigit(line []string) (int, int) {
	first := -1
	last := -1
	...
	fmt.Println("final: ", first, last)
	fmt.Println("----------")

	return first, last
}

// func replaceDigits(phrase string) string {
// 	digits := map[string]int{  
// 		"one": 1,"two": 2,"three": 3, 
// 		"four": 4, "five": 5, "six": 6,
// 		"seven": 7, "eight": 8, "nine": 9,
...
// 		for k, v := range digits {
// 			res1 := strings.Index(new, k) 
// 			if res1 == i {
// 				new = strings.Replace(new, k, strconv.Itoa(v), -1)
// 			}
// 		}
// 	}
// 	// fmt.Println(phrase, " - ", new, "\n")
// 	fmt.Println(phrase)
// 	return new
// }
...
const base = `sqfklfbkjvbghbgmszzzpeightfive2fiveonefive`
// const base = `pcg91vqrfpxxzzzoneightzt
// eightwothree`
